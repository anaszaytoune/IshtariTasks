<?php
class ControllerCatalogAdvancedProduct extends Controller {
	private $error = array();

	public function index() {
		$this->language->load('catalog/product');
		$this->language->load('catalog/advanced_product');
		$this->document->setTitle($this->language->get('heading_title'));

		$this->load->model('catalog/product');
		$this->load->model('catalog/advanced_product');
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->redirect($this->url->link('catalog/advanced_product', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	protected function getList() {
		...
		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('catalog/advanced_product', 'token=' . $this->session->data['token'] . $url, 'SSL'),
			'separator' => ' :: '
		);

		$this->data['insert'] = $this->url->link('catalog/advanced_product/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['copy'] = $this->url->link('catalog/advanced_product/copy', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['delete'] = $this->url->link('catalog/advanced_product/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$product_total = $this->model_catalog_advanced_product->getTotalProducts($data);

		$results = $this->model_catalog_advanced_product->getProducts($data);
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('catalog/advanced_product/update', 'token=' . $this->session->data['token'] . '&product_id=' . $result['product_id'] . $url, 'SSL')
			);
			...
			$categories = $this->model_catalog_advanced_product->getProductCategoriesCustom($result['product_id']);
			$category_ids = array();
			foreach($categories as $value){
				$category_ids[] = $value['category_id'];
			}
			$detailt = $this->model_catalog_advanced_product->getProduct($result['product_id']);
			$this->data['products'][] = array(
				...
				'detailt'    => $detailt,
				'description'=> $this->model_catalog_product->getProductDescriptions($result['product_id'])
			);
		}
		...
		$this->data['text_quickedit'] = $this->language->get('text_quickedit');
		$this->data['text_empty_error'] = $this->language->get('text_empty_error');
		$this->data['text_url_key'] = $this->language->get('text_url_key');
		...
		$this->data['sort_name'] = $this->url->link('catalog/advanced_product', 'token=' . $this->session->data['token'] . '&sort=pd.name' . $url, 'SSL');
		$this->data['sort_model'] = $this->url->link('catalog/advanced_product', 'token=' . $this->session->data['token'] . '&sort=p.model' . $url, 'SSL');
		$this->data['sort_price'] = $this->url->link('catalog/advanced_product', 'token=' . $this->session->data['token'] . '&sort=p.price' . $url, 'SSL');
		$this->data['sort_quantity'] = $this->url->link('catalog/advanced_product', 'token=' . $this->session->data['token'] . '&sort=p.quantity' . $url, 'SSL');
		$this->data['sort_status'] = $this->url->link('catalog/advanced_product', 'token=' . $this->session->data['token'] . '&sort=p.status' . $url, 'SSL');
		$this->data['sort_order'] = $this->url->link('catalog/advanced_product', 'token=' . $this->session->data['token'] . '&sort=p.sort_order' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('catalog/advanced_product', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->template = 'catalog/advanced_product_list.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['product_id'])) {
			$this->data['action'] = $this->url->link('catalog/advanced_product/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$this->data['action'] = $this->url->link('catalog/advanced_product/update', 'token=' . $this->session->data['token'] . '&product_id=' . $this->request->get['product_id'] . $url, 'SSL');
		}

		$this->data['cancel'] = $this->url->link('catalog/advanced_product', 'token=' . $this->session->data['token'] . $url, 'SSL');
	...
	public function quickedit(){
		...
		if(!$this->validateForm()){
			...
			$this->session->data['success'] = sprintf($this->language->get('text_quickedit_success'), $data['name'][$this->config->get('config_language_id')]);
			$this->model_catalog_advanced_product->quickEditProduct($this->request->get['product_id'],$data);
			$data['success'] = 1;
		}
		$this->response->setOutput(json_encode($data));
	}
	protected function validateQuickEdit(){
