<?php
class ModelCatalogAdvancedProduct extends Model {
	...
	public function quickEditProduct($product_id, $data){
		...
		$this->db->query("UPDATE " . DB_PREFIX . "product SET status = '" . $this->db->escape($data['status']) . "' WHERE product_id = '" . (int)$product_id . "'");
		foreach($data['name'] as $key=>$value)
			$this->db->query("UPDATE " . DB_PREFIX . "product_description SET name = '" . $this->db->escape($value) . "' WHERE product_id = '" . (int)$product_id . "' AND language_id='".$key."'");

		$this->db->query("DELETE FROM " . DB_PREFIX . "product_to_category WHERE product_id = '" . (int)$product_id . "'");
		if (isset($data['categories_'.$product_id])) {
			foreach ($data['categories_'.$product_id] as $category_id) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_to_category SET product_id = '" . (int)$product_id . "', category_id = '" . (int)$category_id . "'");
			}
		}
		$this->db->query("DELETE FROM " . DB_PREFIX . "url_alias WHERE query = 'product_id=" . (int)$product_id. "'");
	...
	public function getProductCategoriesCustom($product_id) {
		$product_category_data = array();
		...
		foreach ($query->rows as $key => $result) {
			...
			$product_category_data[$key]['category_id'] = $result['category_id'];
			$product_category_data[$key]['name'] = $category['name'];
		}
